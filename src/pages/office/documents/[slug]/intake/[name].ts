...
import { disposition, contentType, isIntakeFile } from '../../../../../../netlify/functions/lib/office/documents.mjs';

export const GET: APIRoute = async ({ params }) => {
  const slug = params.slug ?? '';
  const name = params.name ?? '';
  if (!SLUG.test(slug) || !isIntakeFile(name)) return new Response('not found', { status: 404 });
